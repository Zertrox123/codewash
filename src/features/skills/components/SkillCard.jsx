...
import MobWithAura from '../../../shared/ui/MobWithAura';
import ParentalVerificationModal from '../../../shared/ui/ParentalVerificationModal';
...
const SkillCard = ({ config, data, themeData, isCenter, isBattling, mobName, mobAura, challenge, damageNumbers, onStartBattle, onEndBattle, onMathSubmit, difficulty, setDifficulty, unlockedDifficulty, selectedBorder, borderColor, bossHealing }) => {
    const [mathInput, setMathInput] = useState('');
    const [isHit, setIsHit] = useState(false);
    const [isWrong, setIsWrong] = useState(false);
    const isReadingWrong = false;
    ...
    const { memoryCards, flippedIndices, matchedPairs, mismatchShake, handleCardClick } = useMemoryGame({
        isBattling,
        skillId: config.id,
        memoryPairs,
        FRIENDLY_MOBS,
                                ...
                                {config.id === 'patterns' ? (
                                    ...
                                    <>
                                        <div className={`flex-1 bg-black/40 rounded border-2 flex items-center justify-center mb-3 p-2 relative overflow-hidden w-full ${isReadingWrong ? 'border-red-500 bg-red-900/30 animate-shake' : 'border-[#555]'}`}>
                                            ...
                                            {config.challengeType === 'reading' && <div className={`absolute bottom-1 text-xs ${isReadingWrong ? 'text-red-400' : 'text-gray-400'}`}>Microphone desactive</div>}
                                        </div>
