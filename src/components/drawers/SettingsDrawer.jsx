...
const SettingsDrawer = ({ isOpen, onReset, bgmVol, setBgmVol, sfxVol, setSfxVol, currentProfile, onSwitchProfile, profileNames, onRenameProfile, getProfileStats, parentStatus, onParentVerified, currentSkills, locale, setLocale, t }) => (
    <div 
        ...
        <div className="p-6 h-full flex flex-col text-slate-200 font-sans">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide py-6">
                <div className="flex flex-col gap-6">
                    <div>
                        <h3 className="text-xl text-blue-300 mb-5 font-bold flex items-center gap-3 uppercase tracking-wider"><Users size={20} /> {t('settings.selectFile')}</h3>
                    ...
                    <div>
                        <h3 className="text-xl text-blue-300 mb-5 font-bold flex items-center gap-3 uppercase tracking-wider"><Music size={20} /> {t('settings.audioConfiguration')}</h3>
                        <div className="space-y-4 bg-slate-900/50 p-5 rounded-xl border-2 border-slate-600">
                            <div className="px-3">
                                <div className="flex justify-between mb-1 text-slate-400 font-bold text-sm uppercase"><span className="pl-2">{t('settings.musicVolume')}</span><span className="text-yellow-400">{Math.round(bgmVol * 100)}%</span></div>
                            ...
                            <div className="px-3">
                                <div className="flex justify-between mb-1 text-slate-400 font-bold text-sm uppercase"><span className="pl-2">{t('settings.sfxVolume')}</span><span className="text-yellow-400">{Math.round(sfxVol * 100)}%</span></div>
                            ...
                            <div className="px-3">
                                <div className="flex justify-between mb-2 text-slate-400 font-bold text-sm uppercase">
                                    <span className="pl-2">{t('chrome.languageLabel')}</span>
                                </div>
                                <select
                                    value={locale}
            ...
            <div className="shrink-0 pt-4 border-t-2 border-red-900/50">
                <h3 className="text-xl text-red-400 mb-4 font-bold flex items-center gap-3 uppercase tracking-wider">
                    <AlertTriangle size={20} className="text-red-500" /> {t('settings.dangerZone')}
                </h3>
                <button onClick={onReset} className="w-full bg-red-950/50 hover:bg-red-900/80 text-red-400 p-3 rounded-lg border border-red-900/50 hover:border-red-500 font-bold text-lg flex items-center justify-center gap-3 transition-all">
                    <Trash2 size={20} /> {t('settings.deleteProfileProgress')}
                </button>
            </div>
        </div>
    </div>
